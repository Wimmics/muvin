class ImageTooltip extends Tooltip{
    ...
    setItemContent(d, id) { // TO-DO: modify according to crobora data
        ...
        let content = `${image}
                    ...
                    ${d.contributors.map(val => `<li title="${val.category}" style="display:flex; gap:10px;"> <img src="${window.location.origin}/muvin/images/${this.chart.app}/${val.category}-icon.svg" width="15px"></img>${capitalizeFirstLetter(val.name)}</li>` ).join('')}
                    </ul>
    ...
    setProfileContent(e, d, id) { 
        ...
        let content = `<img src="${window.location.origin}/muvin/images/${this.chart.app}/${node.type}-icon.svg" width="15px"></img><b> ${node.name}</b><br><br>

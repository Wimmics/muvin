class DataModel {
    ...
    async fetchData(node) {
        const response = await fetch(window.location.origin + '/muvin/data/' + this.chart.app + '?value=' + node.value + '&type=' + node.type)

        return await response.json()
    }

    async fetchNodesLabels(value) {
        const response = await fetch(window.location.origin + '/muvin/data/' + value + '/nodes')
